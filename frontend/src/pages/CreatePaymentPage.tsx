...
const CreatePaymentPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setMessage({ type: 'success', text: `Payment uploaded successfully! Document ID: ${response.document_id}` });
            setXmlContent(''); // Clear the textarea on success
        } catch (error) {
            const errorMessage = error.detail || error.message || 'An unexpected error occurred.';
            setMessage({ type: 'error', text: `Failed to upload payment: ${errorMessage}` });
        } finally {
            setLoading(false);
        }
    };
